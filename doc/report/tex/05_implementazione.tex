...
%
I giochi realizzati sono i quattro descritti nei requisiti, questi sono stati scelti per le loro caratteristiche e per fornire diversi livelli di complessità a livello implementativo.

Si fa ora una breve analisi della struttura di \textit{Connect four} come esempio cardine.
%
Questo poiché la complessità di Connect four è tale da mostrare le potenzialità della libreria senza richiedere un analisi troppo complicata.
%
...
%
Si usa quindi questa \texttt{Feature} appena definita per specificare in quale modo l'azione \texttt{Put} sopra definita agista sullo stato, ovvero, (traducendo quello che in inglese è presente nel codice) piazzando il turno corrente sulla prima cella libera della colonna x.
%
...
\end{itemize}
%
Si nota, nel Listato \ref{lst:connect4_game_description} che i valori definiti sono definiti in modo implicito per semplificare l'utilizzo del \textit{rule set} e del resto della libreria.
